    ...
    <div className="my-5 flex flex-wrap items-center justify-center gap-4 ">
      ...
      {KEYS.map((letter, index) => (
        <button
          ...
          disabled={disabled}
          className={`${
            usedLetters?.includes(letter) || disabled
              ? "bg-gray-600 text-black/60"
              : "bg-[#243642] text-white hover:scale-125 transition duration-300"
          } text-2xl h-12 w-12 rounded-lg`}
        >
          {letter.toUpperCase()}
        </button>
      ))}
